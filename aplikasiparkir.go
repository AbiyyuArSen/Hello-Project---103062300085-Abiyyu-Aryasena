...
func daftarSlotKosong() {
	...
	for i := 0; i < jumlahSlotMaks; i++ {
		if slotParkir[i].PlatNomor == "" {
			kosong = append(kosong, i+1) // Menyimpan slot yang kosong (slot ke-1 sampai ke-10)
		}
	}
	return kosong()
		fmt.Print("Tidak tersedia lahan parkir")
	}
...
func tambahDataParkir() {
	...
	fmt.Println("Slot kosong yang tersedia:")
	for _, slot := range slotKosong{
		fmt.Printf("Slot %d\n", slot)
	}

	var pilihanSlot int
	fmt.Print("Pilih slot yang ingin digunakan (1-10): ")
	fmt.Scanln(&pilihanSlot)

	// Cek apakah slot yang dipilih valid
	if pilihanSlot < 1 || pilihanSlot > jumlahSlotMaks {
		fmt.Println("Slot tidak valid.")
		return
	}

	// Cek apakah slot yang dipilih kosong
	slotIndex := pilihanSlot - 1
	if slotParkir[slotIndex].PlatNomor != "" {
		fmt.Println("Slot yang dipilih sudah terisi. Pilih slot lain.")
		return
	}

	var platNomor, jenis string
	fmt.Print("Masukkan nomor plat kendaraan: ")
	...
	slotParkir[slotIndex] = kendaraan
	fmt.Printf("Kendaraan dengan nomor plat %s diparkir di slot %d\n", platNomor, pilihanSlot)
}
